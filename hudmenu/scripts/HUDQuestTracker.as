package
{
   ...
   public class HUDQuestTracker extends MovieClip
   {
      ...
      {
         var objectiveCount:uint = uint(aQuestData.objectives.length);
         var hasDisplayedObjective:Boolean = false;
         for(var i:uint = 0; i < objectiveCount; i++)
         {
            if(Boolean(aQuestData.objectives[i].isDisplayed) && Boolean(aQuestData.objectives[i].isActive))
            {
      ...
      private function onQuestTrackerData(arEvent:FromClientDataEvent) : void
      {
         ...
         if(Boolean(arEvent) && Boolean(arEvent.data))
         {
            ...
            if(this.isActive)
            {
               shouldSync = Boolean(arEvent.data.shouldSyncQuestTracker);
               if(arEvent.data.questTrackerSyncDelay && this.m_QuestSyncTimer && arEvent.data.questTrackerSyncDelay * 1000 != this.m_QuestSyncTimer.delay)
               {
                  this.m_QuestSyncTimer.delay = arEvent.data.questTrackerSyncDelay * 1000;
               }
               if(shouldSync)
               {
               ...
               else if(!shouldSync && this.m_QuestSyncTimer.running)
               {
                  this.m_QuestSyncTimer.stop();
      ...
      private function initializeObjective(aObjectiveData:Object) : HUDQuestTrackerObjective
      {
         ...
         newObjective.isOffMap = aObjectiveData.isOffMap;
      ...
      private function initializeQuest(aQuestData:Object, aAnimate:Boolean = false) : HUDQuestTrackerEntry
      {
         ...
         for(objectiveKey in aQuestData.objectives)
         {
            ...
            if(objectiveData.state == QUEST_STATE_INPROGRESS && objectiveData.isDisplayed && Boolean(objectiveData.isActive))
            {
      ...
      {
         for(var i:int = int(this.m_DisplayedQuests.length - 1); i >= 0; i--)
         {
      ...
      private function arrangeQuestsObjectives() : void
      {
         for(var i:uint = 0; i < this.m_DisplayedQuests.length; i++)
         {
            ...
            {
               this.m_DisplayedQuests[i].arrangeObjectives(false);
            }
         }
      }
      ...
      private function doProcessQuestUpdateEvents() : uint
      {
         ...
         findQuestEventByID = function(item:*, index:int, array:Array):Boolean
         {
         ...
         findObjEventByID = function(item:*, index:int, array:Array):Boolean
         {
      ...
      private function syncQuestTracker() : void
      {
         ...
         {
            questEvents = new Array();
            this.m_QuestData = BSUIDataManager.GetDataFromClient("QuestTrackerData").data.quests;
            for(i = 0; i < this.m_DisplayedQuests.length; i++)
            {
               questData = this.getQuestDataByID(this.m_DisplayedQuests[i].questID);
               if(!questData || questData && !this.questDataValidForDisplay(questData))
               {
                  questEvents.push({
                     "questID":this.m_DisplayedQuests[i].questID,
            ...
            {
               this.m_PendingQuestEvents = this.m_PendingQuestEvents.concat(questEvents);
               if(this.m_Initialized)
               {
                  this.processNewQuestEventData();
               }
      ...
      private function test_questRemove() : void
      {
         ...
         {
            removedQuest = false;
            for(i = int(this.m_QuestData.length - 1); i >= 0; i--)
            {
               quest = this.m_QuestData[i];
